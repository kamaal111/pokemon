...
def extract_ocr_items(ocr_result: Iterable[OCRResult]) -> list[OCRItem]:
    items: list[OCRItem] = []
...
def choose_best_name_candidate(ocr_items: list[OCRItem]) -> OCRItem | None:
    ...
    def score(item: OCRItem) -> tuple[float, int, int, float]:
        text = item["text"]
...
def choose_best_title_candidate(ocr_items: list[OCRItem]) -> OCRItem | None:
    candidates: list[OCRItem] = []
    for item in ocr_items:
        normalized_text = cleanup_text(item["text"])
...
def choose_best_trainer_candidate(
...
def count_text_support(ocr_items: Sequence[OCRItem], text: str) -> int:
    normalized_text = cleanup_text(text)
    return sum(1 for item in ocr_items if cleanup_text(item["text"]) == normalized_text)


def is_supported_candidate(
    candidate: OCRItem | None, ocr_items: Sequence[OCRItem], min_support: int = 2
) -> bool:
...
def is_confident_candidate(
    candidate: OCRItem | None, min_score: float = MIN_CONFIDENT_SCORE
) -> bool:
    if candidate is None:
        return False
    script_count = len(
...
def should_run_title_ocr(
    ...
    trainer_is_reliable = is_confident_candidate(
        best_trainer_candidate
    ) or is_supported_candidate(
        best_trainer_candidate,
        trainer_items,
    )
    pokemon_is_reliable = is_confident_candidate(
...
def should_prefer_title_for_pokemon(
    ...
    if best_pokemon_candidate is None:
        return True
    if best_trainer_candidate is not None and title_has_trainer_joiner(
        best_title_candidate["text"], language
    ):
        return False
...
def build_variant(
    image: np.ndarray, *, pad_ratio: float, scale: float, transform: str = "color"
) -> np.ndarray:
...
def run(card_image_sample: Path = CARD_IMAGE_SAMPLE, language: str = LANGUAGE) -> dict:
    ...
    used_fallback = not is_confident_candidate(
    ...
    if used_fallback:
        ...
        best_pokemon_candidate = choose_best_name_candidate(
            pokemon_region_items
        ) or choose_best_name_candidate(pokemon_items)
    ...
    if should_drop_trainer_candidate(
        best_title_candidate, best_trainer_candidate, language
    ):
        best_trainer_candidate = None

    trainer_name = (
        best_trainer_candidate["text"] if best_trainer_candidate is not None else None
    )
    pokemon_name = (
        best_pokemon_candidate["text"] if best_pokemon_candidate is not None else None
    )
    combined_name = (
    ...
    print()
    total_seconds = round(perf_counter() - start_total, 3)
    print(f"Total seconds: {total_seconds:.3f}")

    return {
        "extracted_names": extracted_names,
        "best_title_candidate": best_title_candidate,
        "best_trainer_candidate": best_trainer_candidate,
        "best_pokemon_candidate": best_pokemon_candidate,
        "timings": timings,
        "variant_timings": variant_timings,
        "trainer_diagnostics": trainer_diagnostics,
        "pokemon_diagnostics": pokemon_diagnostics,
        "fast_diagnostics": fast_diagnostics,
        "title_items": title_items,
        "pokemon_items": pokemon_items,
        "used_fallback": used_fallback,
        "used_title_ocr_fallback": used_title_ocr_fallback,
